...
import { softDeleteWebThread } from "../actions/webThread";

interface WebThreadListProps {
  thread: Thread;
  profileId: string;
}

const WebThreadList: FC<WebThreadListProps> = ({ thread, profileId }) => {
  const [deleting, setDeleting] = useState(false);
  const router = useRouter();
  const handleSoftDeleteThread = async () => {
    try {
      setDeleting(true);
      const res = await softDeleteWebThread(profileId, thread.id);
      console.log(res);
      setDeleting(false);
      if (res === "Done") {
